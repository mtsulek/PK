...
int main(int argc, char **argv){
    ...
    for(int i=0;i<ilewrzedzie;i++){
        for(int j=0;j<ilewrzedzie;j++){
            czasteczki[biezczast].r=10;//w projekcie 1/10 tej kulki
            czasteczki[biezczast].m=czasteczki[biezczast].r*czasteczki[biezczast].r*0.1;
            czasteczki[biezczast].x=21.0*i;
            czasteczki[biezczast].y=21.0*j;
            ...
            czasteczki[biezczast].vx=g1/0.4;
            czasteczki[biezczast].vy=g2/0.4;//manipulacja temp
            biezczast++;
        }
    }
    double dt=0.01,t=0;
    ...
    double dr=1.0;
    double rho=ilewrzedzie*ilewrzedzie/(romiarpudlax*romiarpudlay);//ilosc kulek na jednostke
    double eta = M_PI*(10.0*2.0)*(10.0*2.0)*rho/4.0;//2.0 to srednica
    double n[10];
    double gr[100];
    ...
    for(;;){
        ...
        if(redraw && al_is_event_queue_empty(event_queue)){
            ...
            al_draw_filled_circle(czasteczki[j].x+50,czasteczki[j].y+50,
            czasteczki[j].r,al_map_rgb(0,255,0));
            for(int j=0;j<liczbaczast;j++){
        ...
        t+=dt;
        cout<<Ek/(ilewrzedzie*ilewrzedzie)<<" \t"<<rho<<" \t"<<eta<<endl;
        al_flip_display();
        }
    }

    int indeks;//indeks histogramu odpowiadajacy odleglosci meidzy 2 czasteczkami
    ...
    for(indeks=0;indeks<100;indeks++){
        gr[indeks]=n[indeks]/(2.0*M_PI*(indeks*dr)*dr);//radialna funkcji dystrybucji
    }
